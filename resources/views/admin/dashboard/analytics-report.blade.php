...
<div class="w-full min-h-screen flex flex-col bg-slate-50/70 p-6 lg:p-10 space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border-2 border-slate-200 shadow-xs">
        <div class="flex items-center gap-3.5">
            ...
            <div>
                <div class="flex items-center gap-2">
                    <h1 class="text-xl lg:text-2xl font-black text-slate-900 tracking-tight">{{ $reportTitle }}</h1>
        ...
        <div class="flex items-center gap-2">
            <span class="px-3.5 py-2 rounded-xl bg-slate-100 border border-slate-200 text-xs font-extrabold text-slate-700 flex items-center gap-2 shadow-2xs">
                ...
                <span>Filtered Total: <span class="text-[#8b1818] font-black" id="filteredCountBadge">{{ method_exists($students ?? null, 'total') ? $students->total() : count($students ?? []) }}</span></span>
            </span>
        </div>
    </div>
    ...
    <div class="bg-white p-6 lg:p-8 rounded-3xl border-2 border-slate-200 shadow-xs w-full">
        ...
        <form method="GET" action="{{ route('admin.analytics.report', 'students') }}" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="section" class="w-full py-2.5 px-3 text-xs font-bold text-slate-800 border-2 border-slate-200 rounded-xl bg-white focus:border-[#8b1818] outline-none cursor-pointer">
                        ...
                        @php
                            // Fallback direct resolution if controller variable is empty
                            $availableSections = $sections ?? \App\Models\User::where('role_id', 3)->whereNotNull('section')->where('section', '!=', '')->distinct()->orderBy('section')->pluck('section');
                        @endphp
            ...
            <div class="flex items-center justify-end gap-3 pt-3">
                <a href="{{ route('admin.analytics.report', 'students') }}" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-extrabold rounded-xl transition text-center">
                    Reset Filters
                </a>
